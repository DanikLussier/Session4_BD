...
ALTER TABLE Clients.Client ADD CONSTRAINT UC_Client_Courriel UNIQUE (Courriel) 
GO
...
BEGIN
	...
	INSERT INTO Clients.Cient (Nom, Prenom, Courriel, MotDePasseHache, MdpSel)
...
BEGIN
	...
	FROM Clients.Client
	WHERE Courriel = @Courriel; -- Les courriels sont uniques

	IF HASHBYTES('SHA2_256', CONCAT(@MotDePasse, @Sel)) = @MdpHache
	BEGIN
		-- On retourne le Client si le mdp est valide
		SELECT * FROM Clients.Client WHERE Courriel = @Courriel;
	END
	ELSE
	BEGIN
		SELECT TOP 0 * FROM Clients.Client; --On ne retourne rien si le mdp est invalide
	END
END
GO
