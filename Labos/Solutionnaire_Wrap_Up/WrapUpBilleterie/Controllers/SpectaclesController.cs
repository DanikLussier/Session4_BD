    ...
    public class SpectaclesController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            return _context.VwSpectaclesRepresentationSpectateurs != null ?
                        ...
                        Problem("Entity set 'R22_BilleterieContext.VwSpectaclesRepresentationSpectateurs'  is null.");
        }
        ...
        public async Task<IActionResult> Details(int? id)
        {
            ...
            // On récupère la vue
            var vue = await _context.VwSpectaclesRepresentationSpectateurs.FirstOrDefaultAsync(x => x.SpectacleId == id);
            if (vue == null)
            {
                return NotFound();
            }

            // On récupère les représentations
            var representations = await _context.Representations.Where(x => x.SpectacleId == id).ToListAsync();

            // On récupère l'image
            var afficheContent = await _context.Affiches
            ...
            // On contruit le ViewModel
            SpectaclesAfficheRepresentationViewModel vm = new SpectaclesAfficheRepresentationViewModel()
            {
                vwSpectacleVue = vue,
