	...
	ALTER TABLE Musique.Chanteur ADD ChanteurID int IDENTITY (1,1) NOT NULL
	GO
	...
	ALTER TABLE Musique.Chanson ADD CONSTRAINT FK_Chanson_ChanteurID FOREIGN KEY (ChanteurID) REFERENCES Musique.Chanteur (ChanteurID)
	GO
	...
	GO
	CREATE FUNCTION Musique.ufn_TrouverIDParNomArtiste(
		@Nom nvarchar(50)
	)
	RETURNS int
	AS
	BEGIN
		RETURN (
			SELECT ChanteurID FROM Musique.Chanteur WHERE Nom = @Nom
		)
	END
	GO

	UPDATE Musique.Chanson
	SET ChanteurID = Musique.ufn_TrouverIDParNomArtiste(NomChanteur)
	GO
